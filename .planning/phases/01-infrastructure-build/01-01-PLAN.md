...
must_haves:
  ...
  artifacts:
    ...
    - path: "packages/core/db/index.ts"
      ...
      exports: ["getProject", "listUserProjects", "createProject"]
  key_links:
    - from: "sst.config.ts"
...
<task type="auto">
  ...
  <done>
    S3 buckets defined with lifecycle policies (90-day artifacts, 30-day → Glacier logs), ready for SST deployment
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    **deployments.ts:**
    - createDeployment(projectId, userId, commitSha): Generate deploymentId, set status to "queued"
    - getDeployment(deploymentId): Query DeploymentIdIndex GSI
    - listProjectDeployments(projectId, userId): Verify project ownership first, then query deployments
