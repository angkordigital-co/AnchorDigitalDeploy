...
must_haves:
  truths:
    ...
    - "User can retrieve deployment history with commit SHA and message (GIT-04)"
  artifacts:
    - path: "infra/webhooks.ts"
      provides: "API Gateway endpoint with webhook and deployments routes"
      exports: ["webhookApi"]
    - path: "packages/functions/webhook-handler/index.ts"
      provides: "GitHub webhook validation and deployment creation"
      exports: ["handler"]
    - path: "packages/functions/deployments-handler/index.ts"
      provides: "Deployment history retrieval with commit metadata"
      exports: ["handler"]
...
<task type="auto">
  <name>Create deployment history API endpoint (GIT-04)</name>
  <files>
    packages/functions/deployments-handler/index.ts
    infra/webhooks.ts
  </files>
  <action>
    Create Lambda handler for deployment history retrieval:
    ...
    **packages/functions/deployments-handler/index.ts:**
    ...
    2. Extract userId from request context (authentication - for now, accept header)
    3. Call listProjectDeployments(projectId, userId) from data access layer
    4. Return deployment list with:
       - deploymentId
       - commitSha (GIT-04: shows commit SHA)
       - commitMessage (GIT-04: shows commit message)
    ...
    **Update infra/webhooks.ts:**
    Add route: GET /projects/{projectId}/deployments → deployments-handler
    - Same IAM permissions as webhook handler (read Projects, read Deployments)
    - Environment variables: PROJECTS_TABLE, DEPLOYMENTS_TABLE

    This fulfills GIT-04: "Deployment history shows Git commit SHA and message"
  </action>
  <verify>
    ```bash
    grep "GET.*deployments" infra/webhooks.ts
    grep "commitSha" packages/functions/deployments-handler/index.ts
    grep "commitMessage" packages/functions/deployments-handler/index.ts
    ```
  </verify>
  <done>
    Deployment history API returns list with commit SHA and message, fulfilling GIT-04 requirement
  </done>
</task>
